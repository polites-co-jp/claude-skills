...
| キー | 内容 |
|---|---|
| `command` | 検証を一括で走らせる単一のコマンド。区分 C のパッケージマネージャから決める。スクリプトを定義できるもの（pnpm・npm・yarn など）は `<パッケージマネージャ> run verify`。それ以外は `make verify` のように、入口を1つ用意する。何を走らせるかの中身は、後続の詳細設計と実装が決める |
